...
class Solomon(T5ForConditionalGeneration):
    ...
    def inter(self, input_ids):
        text_emb = self.shared(input_ids)
        ph_positions = (input_ids == self.ph_token_id).nonzero(as_tuple=False)
        if len(ph_positions) == 0:
            return text_emb 
        projector = MlpProjector().to(self.model_device)
        prev_word_ids = []
        for pos in ph_positions:
            if pos[1] > 0:
                prev_word_ids.append(input_ids[pos[0], pos[1] - 1].item())
        prev_words_int = [int(self.tokenizer.convert_ids_to_tokens(id)) for id in prev_word_ids]
        item_embeddings = torch.stack([self.item_embeds[word] for word in prev_words_int], dim=0).to(self.model_device)
        projected_embeddings = projector(item_embeddings)
        for i, pos in enumerate(ph_positions):
            if pos[1] > 0:
                text_emb[pos[0], pos[1], :] = projected_embeddings[i]
        return text_emb        
    ...
    def init_prompt(self, task_num, prompts_per_task, device):
        ...
        self.ph_token_id = 8023
        self.projector = MlpProjector().to(self.model_device)
        
        # 加载 item_embeds 只进行一次
        with open('SASRec_item_embed.pkl', 'rb') as f:
            self.item_embeds = pickle.load(f)
    ...
    def input_plus_whole_word(self,task_id,input_ids, whole_word_ids):
        if task_id[0].item() in [1]:
            text_emb=self.inter(input_ids)
        else:    
    ...
    def append_prompt(self, task_id, input_ids, whole_word_ids, attention_mask):
        ...
        # text
        text_emb_plus = self.input_plus_whole_word(task_id,input_ids, whole_word_ids)
