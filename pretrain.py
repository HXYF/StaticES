...
def train():
    ...
    total_sample = 0
    while True:        
        task, source, source_mask, whole_word, user,target = all_iterator.next_batch()
        ...
        optimizer.zero_grad()
        outputs = model(task, source, whole_word, source_mask, user, labels=target)
        loss = outputs.loss
        loss.backward()
        optimizer.step()
...
def evaluate(iterator):
    ...
    with torch.no_grad():
        while True:
            task, source, source_mask, whole_word, user,target = iterator.next_batch_valid()
            task = task.to(device)  # (batch_size,)
            source = source.to(device)  # (batch_size, seq_len)
            source_mask = source_mask.to(device)
            whole_word = whole_word.to(device)
